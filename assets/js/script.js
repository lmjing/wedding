...
const invitationSlug = window.location.pathname.split('/').filter(Boolean)[0] || '';
...
function preloadImages(imageUrls) {
    ...
    imageUrls.forEach((url, index) => {
        ...
        img.onload = function() {
            ...
            console.log(`✅ 이미지 로드 완료 (${loadedCount}/${totalImagesToPreload}): ${url}`);
...
function triggerVisibleAnimations() {
    ...
    const animationElements = document.querySelectorAll('.fade-in, .fade-in-up, .fade-in-left, .fade-in-right, .fade-in-scale, .fade-in-spring');
...
function updateCountdownDisplay(days, hours, minutes, seconds) {
    ...
    if (countdown) {
        countdown.innerHTML = `
            <div><span class="card">${days.toString().padStart(2, '0')}</span><div class="desc">Days</div></div>
            <div><span>:</span></div>
            <div><span class="card">${hours.toString().padStart(2, '0')}</span><div class="desc">Hour</div></div>
            <div><span>:</span></div>
            <div><span class="card">${minutes.toString().padStart(2, '0')}</span><div class="desc">Min</div></div>
            <div><span>:</span></div>
            <div><span class="card">${seconds.toString().padStart(2, '0')}</span><div class="desc">Sec</div></div>
        `;
    }
...
function initRsvp() {
    ...
    form.addEventListener('submit', function(event) {
        ...
        const side = (form.querySelector('input[name="rsvp_side"]:checked') || {}).value || 'groom';
        ...
        const companion = (form.querySelector('#rsvp-companion') || {}).value.trim();
        const meal = (form.querySelector('input[name="rsvp_meal"]:checked') || {}).value || 'planned';
    ...
    if (audioPlayer && audioAlert) {
        ...
        if (window.backgroundMusicConfig) {
            ...
            if (config.autoplay) {
                ...
                const userInteractionEvents = ['click', 'touchstart', 'touchend', 'mousedown', 'keydown'];
        ...
        audioPlayer.addEventListener('loadeddata', function() {
            ...
            if (window.backgroundMusicConfig && window.backgroundMusicConfig.autoplay && audioPlayer.paused) {
...
function initIntroVideo() {
    ...
    if (introPlayer) {
        ...
        introPlayer.addEventListener('canplaythrough', function() {
            introPlayer.play().catch(e => console.log('Intro video play failed:', e));
        });
...
function openImageModal(imageSrc) {
    // 모든 갤러리 이미지 수집
    const galleryItems = document.querySelectorAll('.gallery-grid .grid-item .item');
    const galleryImages = [];
            ...
            <div class="gallery-modal-container">
                <div class="gallery-modal-images" id="gallery-modal-images">
                    ${galleryImages.map((src, index) => `
                        <div class="gallery-modal-item ${index === currentImageIndex ? 'active' : ''}" data-index="${index}">
                ...
                <div class="gallery-modal-indicator">
                    ${galleryImages.map((_, index) => `
                        <span class="gallery-modal-dot ${index === currentImageIndex ? 'active' : ''}" data-index="${index}"></span>
                    `).join('')}
                </div>
...
function closeGalleryModal(element) {
    const modal = element ? element.closest('.modal-mask') : document.querySelector('.modal-mask.gallery, .modal-mask.gallery-swipe-modal');
    if (modal) {
        // 스크롤 위치 복원
...
function openKakaoNavi() {
    ...
    if (/Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
        ...
        const fallbackUrl = `https://map.kakao.com/link/search/${encodeURIComponent(address)}`;
...
function openTmap() {
    ...
    // 모바일에서 티맵 앱 실행 시도
    if (/Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
        const tmapUrl = `tmap://search?name=${encodeURIComponent(placeName)}&address=${encodeURIComponent(address)}`;
        const fallbackUrl = `https://tmap.life/route/search?name=${encodeURIComponent(placeName + ' ' + address)}`;
        ...
        // 데스크톱에서는 티맵 웹으로 이동
        const webUrl = `https://tmap.life/route/search?name=${encodeURIComponent(placeName + ' ' + address)}`;
        window.open(webUrl, '_blank');
    }
}
        ...
        if (typeof Kakao !== 'undefined' && Kakao.isInitialized()) {
            ...
            await Kakao.Share.sendDefault({
                ...
                content: {
                    title,
                    description: desc,
                    imageUrl: window.location.origin + '/assets/images/20250918_225238_2.png',
...
function initZoomableMap() {
    ...
    if (mapContainerParent) {
        mapContainerParent.addEventListener('touchstart', function(event) {
            if (event.touches.length > 1) {
                event.preventDefault();
        ...
        mapContainerParent.addEventListener('touchmove', function(event) {
            if (event.touches.length > 1) {
                event.preventDefault();
                event.stopPropagation();
            }
        ...
        mapContainerParent.addEventListener('gesturestart', function(event) {
            event.preventDefault();
        }, { passive: false });
        
        mapContainerParent.addEventListener('gesturechange', function(event) {
            event.preventDefault();
        }, { passive: false });
        
        mapContainerParent.addEventListener('gestureend', function(event) {
            event.preventDefault();
        }, { passive: false });
    }
